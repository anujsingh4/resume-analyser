...
function ImportanceBadge({ level }) {
  ...
  return (
    <span className={`badge ${colors[level] || "badge-low"}`}>{level}</span>
  );
}
  ...
  const handleAnalyse = async () => {
    ...
    try {
      const res = await fetch(
        "https://resume-analyser-backend-60nd.onrender.com/api/ai-analyse",
        {
          method: "POST",
          body: formData,
        },
      ...
      setError(
        "Cannot connect to server. Is your backend running on port 8080?",
      );
    } finally {
      setLoading(false);
    }
        ...
        >
          {loading ? (
            <span className="loading-text">
              <span className="spinner" /> Analysing with AI — this takes ~15
              seconds...
            </span>
          ) : (
            "Analyse My Resume →"
        ...
        {result && (
          <div className="result-box">
                ...
                <div className="stats-grid">
                  <div className="stat">
                    <span className="stat-num">
                      {result.matchResult.totalJobKeywords}
                    </span>
                    <span className="stat-label">Keywords in JD</span>
                  </div>
                  <div className="stat">
                    <span className="stat-num matched-num">
                      {result.matchResult.matchedCount}
                    </span>
                    <span className="stat-label">Matched</span>
                  </div>
                  <div className="stat">
                    <span className="stat-num missing-num">
                      {result.matchResult.missingCount}
                    </span>
            ...
            <div className="pill-list">
              {activeTab === "matched"
                ? result.matchResult.topMatched.map((w) => (
                    <Pill key={w} word={w} type="matched" />
                  ))
                : result.matchResult.topMissing.map((w) => (
                    <Pill key={w} word={w} type="missing" />
                  ))}
            </div>
            ...
            {result.aiResult && (
              <div className="ai-section">
                <div className="ai-header">
                  ...
                  <span
                    className={`verdict verdict-${result.aiResult.fitVerdict?.replace(/\s+/g, "-").toLowerCase()}`}
                  >
                ...
                <div className="tabs">
                  <button
                    className={`tab ${aiTab === "summary" ? "active" : ""}`}
                    onClick={() => setAiTab("summary")}
                  >
                    Summary
                  </button>
                  <button
                    className={`tab ${aiTab === "gaps" ? "active" : ""}`}
                    onClick={() => setAiTab("gaps")}
                  >
                    Skill Gaps
                  </button>
                  <button
                    className={`tab ${aiTab === "improvements" ? "active" : ""}`}
                    onClick={() => setAiTab("improvements")}
                  >
                    Improvements
                  </button>
                </div>
                ...
                {aiTab === "summary" && (
                  <div className="ai-content">
                    <p className="ai-summary">
                      {result.aiResult.overallSummary}
                    </p>
                  ...
                  <div className="ai-content">
                    {result.aiResult.resumeImprovements?.map((item, i) => (
                      <div key={i} className="improvement-card">
                        <span className="improvement-section">
                          {item.section}
                        </span>
            ...
            {result.matchResult.topMissing.length > 0 && (
              <div className="tip-box">
                💡 <strong>Tip:</strong> Add the missing keywords naturally into
                your resume where they genuinely apply.
              </div>
            )}
          </div>
        )}
